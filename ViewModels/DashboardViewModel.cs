...
public class DashboardViewModel : ViewModelBase
{
    ...
    private string _masterPasswordStatusText = string.Empty;
    public string MasterPasswordStatusText
    {
        get => _masterPasswordStatusText;
        set => SetField(ref _masterPasswordStatusText, value);
    }

    private bool _isMasterPasswordSet;
    public bool IsMasterPasswordSet
    {
        get => _isMasterPasswordSet;
        set => SetField(ref _isMasterPasswordSet, value);
    }

    private string _autoLockStatusText = string.Empty;
    public string AutoLockStatusText
    {
        get => _autoLockStatusText;
        set => SetField(ref _autoLockStatusText, value);
    }
    ...
    public bool IsAutoLockEnabled
    {
        ...
        set => SetField(ref _isAutoLockEnabled, value);
    }
    ...
    public ICommand LockAppCommand { get; }
    ...
    private async Task LoadSecurityRemindersAsync()
    {
        try
        {
            ...
            var masterPasswordHash = await _settingsRepository.GetValueAsync(SettingKeys.MasterPasswordHash);
            if (!string.IsNullOrEmpty(masterPasswordHash))
            {
                MasterPasswordStatusText = GetLocalizedString("Lang.Dashboard.MasterPasswordSet", "Master password set");
                IsMasterPasswordSet = true;
            }
            else
            {
                MasterPasswordStatusText = GetLocalizedString("Lang.Dashboard.MasterPasswordNotSet", "Master password not set");
                IsMasterPasswordSet = false;
            }

            var autoLockEnabled = await _settingsRepository.GetValueAsync(SettingKeys.IdleAutoLockEnabled);
            if (autoLockEnabled == "True")
            {
                AutoLockStatusText = GetLocalizedString("Lang.Dashboard.AutoLockEnabled", "Auto-lock enabled");
                IsAutoLockEnabled = true;
            }
            else
            {
                AutoLockStatusText = GetLocalizedString("Lang.Dashboard.AutoLockDisabled", "Auto-lock disabled");
