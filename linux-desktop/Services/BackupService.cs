    ...
    public async Task<ImportResult> ImportAsync(BackupFile backupFile, string backupPassword, ImportMode mode, ConflictStrategy conflictStrategy)
    {
        ...
        try
        {
            ...
            if (expectedSignature != backupFile.Signature)
            {
                ...
                result.ErrorMessage = GetLang("Lang.Backup.SignatureVerifyFailed", "数据签名验证失败，文件可能已被篡改或密码错误");
                return result;
            }
            ...
            {
                result.Success = false;
                result.ErrorMessage = GetLang("Lang.Backup.DecryptFailed", "解密失败，密码错误");
                return result;
            }
            ...
            {
                result.Success = false;
                result.ErrorMessage = GetLang("Lang.Backup.DataFormatError", "备份数据格式错误");
                return result;
            }
            ...
            foreach (var backupAccount in backupData.Accounts)
            {
                if (mode == ImportMode.Merge && existingSecrets.Contains(backupAccount.Secret))
                {
                    ...
                    {
                        backupAccount.Name += GetLang("Lang.Backup.RenameSuffix", " (导入)");
            ...
            foreach (var backupCategory in backupData.Categories)
            {
                if (mode == ImportMode.Merge && existingCategoryNames.Contains(backupCategory.Name))
                {
                    ...
                    {
                        backupCategory.Name += GetLang("Lang.Backup.RenameSuffix", " (导入)");
        ...
        {
            result.Success = false;
            result.ErrorMessage = string.Format(GetLang("Lang.Backup.ImportFailedFormat", "导入失败: {0}"), ex.Message);
        }
